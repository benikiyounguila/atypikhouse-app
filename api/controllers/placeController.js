...
exports.getPlaces = async (req, res) => {
  try {
    ...
    const [places, total] = await Promise.all([
      Place.find(query).skip(skip).limit(limit),
...
exports.singlePlace = async (req, res) => {
  try {
    ...
    // Vérifier si le logement est actif (sauf pour les propriétaires et admins)
    if (!place.isActive && req.user.role !== 'owner' && req.user.role !== 'admin') {
      return res.status(400).json({
