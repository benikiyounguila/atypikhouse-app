...
const config = {
  baseUrl: "https://atypikhouse.onrender.com",
...
// Fonction pour parcourir les répertoires et trouver les fichiers
...
async function generateSitemap() {
  ...
  try {
    ...
    const files = getFiles(config.pagesDir);
    ...
    // Générer le XML du sitemap brut
    const rawSitemap = await streamToPromise(
      Readable.from(links).pipe(stream)
    ).then((data) => data.toString());

    // Formater le XML pour l’indentation et la lisibilité
    const sitemap = xmlFormatter(rawSitemap, {
      indentation: "  ",   // 2 espaces pour l'indentation
      collapseContent: true,
    });

    console.log("Contenu du sitemap formaté :");
    console.log(sitemap);
