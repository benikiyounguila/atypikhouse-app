...
const PlacesFormPage = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
          axiosInstance.get(`/owner/places/${id}`).then((response) => {
  ...
  const savePlace = async (e) => {
    ...
    if (formDataIsValid) {
      try {
        if (id) {
          const { data } = await axiosInstance.put(`/owner/places/${id}`, placeData);
        } else {
          const { data } = await axiosInstance.post('/owner/places', placeData);
        }
        setRedirect(true);
      } catch (error) {
  ...
  if (redirect) {
    return <Navigate to={'/owner/places'} />;
  }
  ...
  return (
    <div className="p-4">
      ...
      <form onSubmit={savePlace}>
        ...
        <select name="type" value={type} onChange={handleFormData} required className="w-full p-2 border rounded-md shadow-sm focus:ring-primary focus:border-primary">
        ...
        <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-4">
          <div>
            <h3 className="-mb-1 mt-2">Nombre maximum</h3>
            <input
              type="text"
              name="maxGuests"
              value={maxGuests}
        ...
        <button disabled={isUploading} className="mx-auto my-4 flex rounded-full bg-primary px-20 py-3 text-xl font-semibold text-white disabled:bg-gray-400">
