...
const AccountNav = () => {
  ...
  if (pathname.includes('/account') && subpage === undefined) {
    subpage = 'profile';
  } else if (pathname.includes('/admin') && subpage === 'dashboard') {
    subpage = 'dashboard';
  } else if (pathname.includes('/admin') && subpage === 'comments') {
    subpage = 'comments';
  }
  ...
  return (
    <nav className="mb-8 mt-24 flex w-full flex-col justify-center gap-2 p-8 md:flex-row md:p-0">
      ...
      {user?.role === 'modérateur' && (
        <Link className={linkClases('comments')} to={'/admin/comments'}>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
